        ...
        if str(rank) in FileRank_to_GoalRank:
            ...
            for i in range(len(nsys_rank_events)):
                ...
                if nsys_rank_events[i]["event_algo"] == "Tree":
                    used_for_mapping_gpu_prim_event = None
                    for channel, tid_gpu_prim_events in channel_gpu_prim_events.items():
                        for tid, gpu_prim_events in tid_gpu_prim_events.items():
                            if used_for_mapping_gpu_prim_event == None:
                                used_for_mapping_npkit_gpu_event_id = next_npkit_event_id[rank][channel][tid]
                                used_for_mapping_gpu_prim_event = gpu_prim_events[used_for_mapping_npkit_gpu_event_id]
                            else:
                                compare_used_for_mapping_npkit_gpu_event_id = next_npkit_event_id[rank][channel][tid]
                                compare_used_for_mapping_gpu_prim_event = gpu_prim_events[compare_used_for_mapping_npkit_gpu_event_id]
                                if compare_used_for_mapping_gpu_prim_event["ts_end"] - compare_used_for_mapping_gpu_prim_event["ts_start"] > used_for_mapping_gpu_prim_event["ts_end"] - used_for_mapping_gpu_prim_event["ts_start"]:
                                    used_for_mapping_gpu_prim_event = compare_used_for_mapping_gpu_prim_event

                    gpu_prim_event_ts_start = used_for_mapping_gpu_prim_event["ts_start"]
                    gpu_prim_event_ts_end = used_for_mapping_gpu_prim_event["ts_end"]
                    ...
                    for channel, tid_gpu_prim_events in channel_gpu_prim_events.items():
                        ...
                        for tid, gpu_prim_events in tid_gpu_prim_events.items():
                            ...
                            offset = gpu_prim_event_ts_start - gpu_prim_event["ts_start"]
                            # gpu_prim_event["ts_start"] = nsys_nccl_kernel_ts_start
                            # gpu_prim_event["ts_end"] = nsys_nccl_kernel_ts_end
                            gpu_prim_event["ts_start"] = map_p_to_t(gpu_prim_event["ts_start"] + offset, gpu_prim_event_ts_start, gpu_prim_event_ts_end, nsys_nccl_kernel_ts_start, nsys_nccl_kernel_ts_end)
                            gpu_prim_event["ts_end"] = map_p_to_t(gpu_prim_event["ts_end"] + offset, gpu_prim_event_ts_start, gpu_prim_event_ts_end, nsys_nccl_kernel_ts_start, nsys_nccl_kernel_ts_end)

                            for j in range(len(gpu_prim_event["prim_events"])):
                                gpu_prim_event["prim_events"][j]["ts_start"] = map_p_to_t(gpu_prim_event["prim_events"][j]["ts_start"] + offset, gpu_prim_event_ts_start, gpu_prim_event_ts_end, nsys_nccl_kernel_ts_start, nsys_nccl_kernel_ts_end)
                                gpu_prim_event["prim_events"][j]["ts_end"] = map_p_to_t(gpu_prim_event["prim_events"][j]["ts_end"] + offset, gpu_prim_event_ts_start, gpu_prim_event_ts_end, nsys_nccl_kernel_ts_start, nsys_nccl_kernel_ts_end)
                                gpu_prim_event["prim_events"][j]["ts_calc"] = map_p_to_t(gpu_prim_event["prim_events"][j]["ts_calc"] + offset, gpu_prim_event_ts_start, gpu_prim_event_ts_end, nsys_nccl_kernel_ts_start, nsys_nccl_kernel_ts_end)
