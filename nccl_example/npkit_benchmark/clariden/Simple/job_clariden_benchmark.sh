...
srun --mpi=pmi2 --environment=megatron bash -c '
    if [ "$SLURM_NODEID" -eq 0 ]; then
        ...
        nvcc -I${MPI_ROOT}/include -L${MPI_ROOT}/lib -lmpi -I${NCCL_ROOT}/include -L${NCCL_ROOT}/lib -lnccl /users/zhu/DistributedSysLab_FS24/nccl_example/npkit_benchmark/clariden/Simple/example_allreduce.cu -o /users/zhu/DistributedSysLab_FS24/nccl_example/npkit_benchmark/clariden/Simple/example_allreduce
...
for index in $(seq 0 9); do
    iteration_dir="/users/zhu/DistributedSysLab_FS24/nccl_example/npkit_benchmark/clariden/Simple/results/${index}"
    ...
    while [ $size -le $((4 * 1024 * 1024)) ]; do
        ...
        srun --mpi=pmi2 --environment=megatron bash -c "
            ...
            /users/zhu/DistributedSysLab_FS24/nccl_example/npkit_benchmark/clariden/Simple/example_allreduce $size | tee $npkit_result_dir/log.txt
        "

        # Process NPKit trace
        # srun --mpi=pmi2 --environment=megatron bash -c '
        #     if [ "$SLURM_NODEID" -eq 0 ]; then
        #         python3 /users/zhu/DistributedSysLab_FS24/nccl_example/npkit_benchmark/clariden/Simple/npkit_dependency_trace_generator.py --npkit_dump_dir=$npkit_dump_dir \
...
srun --mpi=pmi2 --environment=megatron bash -c '
    if [ "$SLURM_NODEID" -eq 0 ]; then
        pip install seaborn
        python3 /users/zhu/DistributedSysLab_FS24/nccl_example/npkit_benchmark/clariden/Simple/plot.py
        cp -r ./plots /users/zhu/DistributedSysLab_FS24/nccl_example/npkit_benchmark/clariden/Simple
    fi
'
