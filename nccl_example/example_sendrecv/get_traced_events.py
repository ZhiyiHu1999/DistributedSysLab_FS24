...
def get_nsys_events(dir_path):
    ...
    for file_name in os.listdir(dir_path):  ## each file represents a rank
        ...
        if file_name.endswith('.sqlite'):
            ...
            pattern_ring = r"Rings \[(\d+)\] (\d+)->(\d+)->(\d+)"
            pattern_tree = r"Trees \[(\d+)\] (\d+)->(\d+)->(\d+)->(\d+)->(\d+)"
...
def main():
    ...
    Nsys_Events, FileRank_2_GoalRank, HostName_2_GoalRank, GoalRank_2_NumOfRanks  = get_nsys_events(Dir_Path)
